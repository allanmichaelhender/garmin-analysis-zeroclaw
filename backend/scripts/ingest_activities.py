    ...
    try:
        ...
        for raw_activity in activities:
            ...
            if existing:
                ...
                db.add(activity)
                ingested_count += 1
                logger.info(f"Ingested activity {activity_id} - {activity_data['activity_type']}")

            # Commit each activity individually to avoid parameter conflicts
            db.commit()
            db.expunge_all()  # Clear session to avoid parameter conflicts

        logger.info(f"Successfully ingested {ingested_count} new activities, updated {updated_count} existing activities")
